...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Función para mostrar el mensaje de bienvenida o de error
  function mostrarMensaje(nombre) {
      if (validarNombre(nombre)) {
          // Mostrar mensaje de bienvenida utilizando alert
          alert(`¡Bienvenido a la página de fans de Dread Mar-i, ${nombre}!`);
      } else {
          // Mostrar mensaje de error si el nombre no es válido
          alert('Por favor, ingresa un nombre válido de al menos tres letras y solo letras del alfabeto.');
  ...
  // Llamar a la función para solicitar el nombre al usuario
  mostrarMensajeBienvenida();
});
